...
export default function VideoSummary({ playbackId }: { playbackId: string }) {
    ...
    return (
        <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-purple-500 hover:bg-purple-600 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
        >
            {isGenerating ? (
                <>
                ...
                <>
                    ...
                    {error ? 'Retry AI summary' : 'Generate AI summary'}
                </>
            )}
        </button>
    );
